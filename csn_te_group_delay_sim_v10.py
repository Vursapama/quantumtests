...
def make_plots(results: List[Dict[str, float]], base: SimParams, rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    ...
    for ax, d in zip(axes, rep_dets):
        ...
        i_in_plot_n = safe_norm(i_in_plot)
        i_out_plot_n = safe_norm(i_out_plot)
        exc_plot_n = safe_norm(exc_plot)
        te_plot_n = safe_norm(te_plot)

        ax.plot(t_plot, i_in_plot_n, label='input (norm)')
        ax.plot(t_plot, i_out_plot_n, '--', label='output (norm)')
        ax.plot(t_plot, exc_plot_n, ':', label='excitation (norm)')
        ax.plot(t_plot, te_plot_n, '-.', label='TE memory (norm)')
