...
def run_trace(params: SimParams) -> Dict[str, np.ndarray]:
    w = np.linspace(-params.w_max, params.w_max, params.n_freq)
    t = time_grid_from_freq(w)
    e_in_w = gaussian_spectrum(w, params.delta0, params.sigma)
    e_in_t = ifft_shifted(e_in_w)
    ...
    e_out_t = ifft_shifted(e_out_w)
    exc_t, te_current, te_memory = solve_atomic_excitation(
        t, e_in_t, params.gamma, params.delta0, params.kappa,
        params.memory_decay, params.memory_gain,
        params.te_delta_feedback, params.te_kappa_feedback,
    )
    return {
        "t": t,
        "i_in": np.abs(e_in_t) ** 2,
...
def compute_plot_window(t: np.ndarray, i_in: np.ndarray, exc: np.ndarray, te_memory: np.ndarray, frac: float = 0.01, pad: int = 250) -> slice:
    combo = np.maximum.reduce([np.maximum(i_in, 0.0), np.maximum(exc, 0.0), np.maximum(te_memory, 0.0)])
    peak = float(np.max(combo)) if combo.size else 0.0
    if peak <= 1e-15:
        return slice(0, len(t))
    idx = np.where(combo >= frac * peak)[0]
    if idx.size == 0:
        return slice(0, len(t))
    lo = max(0, int(idx[0]) - pad)
    hi = min(len(t), int(idx[-1]) + pad + 1)
    return slice(lo, hi)
...
def make_plots(results: List[Dict[str, float]], base: SimParams, rows: List[Dict[str, float]], out_dir: str) -> List[str]:
    os.makedirs(out_dir, exist_ok=True)
    paths: List[str] = []

    det = np.array([r['delta0'] for r in results])
    ...
    axes[0].plot(det, tau_phase, label='phase')
    axes[0].plot(det, tau_cent, label='centroid')
    axes[0].plot(det, tau_peak, label='peak')
    axes[0].plot(det, tau_exc, label='excitation')
    axes[0].set_xlabel('detuning')
    axes[0].set_ylabel('delay')
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    axes[1].plot(det, r_phase, label='|R_phase|')
    axes[1].plot(det, r_cent, label='|R_centroid|')
    axes[1].plot(det, r_peak, label='|R_peak|')
    axes[1].axhline(1.0, linestyle='--')
    axes[1].set_xlabel('detuning')
    axes[1].set_ylabel('|tau_g / tau_exc|')
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)
    p1 = os.path.join(out_dir, 'figure1_delay_and_ratio.png')
    fig.savefig(p1, dpi=150)
    plt.close(fig)
    paths.append(p1)

    decays = sorted(set(r['memory_decay'] for r in rows))
    gains = sorted(set(r['memory_gain'] for r in rows))
    fig, axes = plt.subplots(2, 1, figsize=(8, 8), constrained_layout=True)
    for md in decays:
        sub = [r for r in rows if abs(r['memory_decay'] - md) < 1e-12]
        sub = sorted(sub, key=lambda x: x['memory_gain'])
        axes[0].plot([r['memory_gain'] for r in sub], [r['phase_abs_ratio_mean'] for r in sub], label=f'decay={md:.2f}')
        axes[1].plot([r['memory_gain'] for r in sub], [r['te_memory_peak_mean'] for r in sub], label=f'decay={md:.2f}')
    axes[0].set_xlabel('memory_gain')
    axes[0].set_ylabel('phase_abs_ratio_mean')
    axes[0].grid(True, alpha=0.3)
    axes[0].legend(fontsize=8)
    axes[1].set_xlabel('memory_gain')
    axes[1].set_ylabel('te_memory_peak_mean')
    axes[1].grid(True, alpha=0.3)
    axes[1].legend(fontsize=8)
    p2 = os.path.join(out_dir, 'figure2_memory_sweep.png')
    fig.savefig(p2, dpi=150)
    plt.close(fig)
    paths.append(p2)

    rep_dets = [-2.0, 0.0, 2.0]
    fig, axes = plt.subplots(3, 1, figsize=(8, 10), constrained_layout=True)
    for ax, d in zip(axes, rep_dets):
        p = SimParams(
        ...
        tr = run_trace(p)
        sl = compute_plot_window(tr['t'], tr['i_in'], tr['exc'], tr['te_memory'], frac=0.005, pad=400)
        t_plot = tr['t'][sl]
        i_in_plot = tr['i_in'][sl]
        i_out_plot = tr['i_out'][sl]
        exc_plot = tr['exc'][sl]
        te_plot = tr['te_memory'][sl]

        exc_scale = 1.0
        te_scale = 1.0
        peak_ref = max(float(i_in_plot.max()), float(i_out_plot.max()), 1e-12)
        if float(exc_plot.max()) > 1e-12:
            exc_scale = 0.8 * peak_ref / float(exc_plot.max())
        if float(te_plot.max()) > 1e-12:
            te_scale = 0.8 * peak_ref / float(te_plot.max())

        ax.plot(t_plot, i_in_plot, label='input')
        ax.plot(t_plot, i_out_plot, '--', label='output')
        ax.plot(t_plot, exc_scale * exc_plot, ':', label=f'exc ×{exc_scale:.1f}')
        ax.plot(t_plot, te_scale * te_plot, '-.', label=f'TE mem ×{te_scale:.1f}')
        ax.set_title(f'delta = {d:+.1f}')
        ax.set_ylabel('amplitude')
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
    axes[-1].set_xlabel('time (zoomed to pulse support)')
    p3 = os.path.join(out_dir, 'figure3_time_domain_traces.png')
...
def main() -> None:
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'group_delay_figures')
