...
def make_plots(results: List[Dict[str, float]], base: SimParams, output_dir: str | None = None) -> List[str]:
    ...
    ax.legend()
    sweep_path = str(output_root / f"{prefix}_sweep.png")
    fig.tight_layout()
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    ratio_path = str(output_root / f"{prefix}_ratios.png")
    fig.tight_layout()
    ...
    for d in reps:
        ...
        ax.grid(True, alpha=0.3)
        ax.legend()
        trace_name = f"{prefix}_trace_{d:+.2f}.png".replace("+", "p").replace("-", "m")
        trace_path = str(output_root / trace_name)
        fig.tight_layout()
